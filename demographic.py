...
def main():
    ...
    for method in ["ASM", "SP", "RS", "CC", "OVA", "DT", "LCE"]:
        raws = pd.read_csv("resultsRAW/chestxray2/GCresultsRAW_test_chestxray2_{}_42_ep3.csv".format(method))

        tmp = pd.concat([df, raws], axis=1)
        cols = ["Image Index", "Patient ID", "Patient Age", "Patient Gender", 'rej_score', 'labels',
                'hum_preds', 'preds', 'class_probs_0', 'class_probs_1']
        tmp = tmp[cols].copy()
        tmp.to_csv(
            "resultsRAW/chestxray2/{}_Scenario1_Conditional_test_chestxray{}_42_ep3.csv".format(method, label_chosen),
            index=False)
