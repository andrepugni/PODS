...
class GenericDatasetDeferral(BaseDataset):
    ...
    def generate_data(self):
        ...
        self.data_train_loader = torch.utils.data.DataLoader(
            self.data_train, batch_size=self.batch_size, shuffle=True, num_workers=8, pin_memory=True
        )
        self.data_val_loader = torch.utils.data.DataLoader(
            self.data_val, batch_size=self.batch_size, shuffle=False, num_workers=8, pin_memory=True
        )
        self.data_test_loader = torch.utils.data.DataLoader(
            self.data_test, batch_size=self.batch_size, shuffle=False, num_workers=8, pin_memory=True
        )
...
class SyntheticData(BaseDataset):
    ...
    def __init__(self, train_samples=1000, test_samples=1000, data_distribution="mix_of_guassians", d=10,
                 mean_scale=1, expert_deferred_error=0, expert_nondeferred_error=0.5, machine_nondeferred_error=0,
                 num_of_guassians=10,
    ...
    def generate_data(self):
        ...
        self.train_x, self.val_x, self.test_x = torch.utils.data.random_split(data_x, [train_size, val_size, test_size],
                                                                              generator=torch.Generator().manual_seed(
                                                                                  42))
        self.train_y, self.val_y, self.test_y = torch.utils.data.random_split(data_y, [train_size, val_size, test_size],
                                                                              generator=torch.Generator().manual_seed(
                                                                                  42))
        self.train_h, self.val_h, self.test_h = torch.utils.data.random_split(human_predictions,
                                                                              [train_size, val_size, test_size],
                                                                              generator=torch.Generator().manual_seed(
                                                                                  42))
        ...
        self.data_train = torch.utils.data.TensorDataset(self.train_x.dataset.data[self.train_x.indices],
                                                         self.train_y.dataset.data[self.train_y.indices],
                                                         self.train_h.dataset.data[self.train_h.indices])
        self.data_val = torch.utils.data.TensorDataset(self.val_x.dataset.data[self.val_x.indices],
                                                       self.val_y.dataset.data[self.val_y.indices],
                                                       self.val_h.dataset.data[self.val_h.indices])
        self.data_test = torch.utils.data.TensorDataset(self.test_x.dataset.data[self.test_x.indices],
        ...
        self.data_val_loader = torch.utils.data.DataLoader(self.data_val, batch_size=self.batch_size, shuffle=False)
        self.data_test_loader = torch.utils.data.DataLoader(self.data_test, batch_size=self.batch_size, shuffle=False)
...
class GalaxyZoo(BaseDataset):
    ...
    def generate_data(self):
        ...
        if not os.path.exists(
                self.data_dir
                + "/galaxyzoo/training_solutions_rev1.csv"
        ):
            raise ValueError(
        ...
        if not os.path.exists(
                self.data_dir + "/galaxyzoo/images_training_rev1"
        ):
            raise ValueError(
                "cant find csv, Please download the data from https://www.kaggle.com/c/galaxy-zoo-the-galaxy-challenge/overview"
                " , unzip it, and construct the path of the folder in data_dir" )
        ...
        df["hum_pred"] = df["Class1.1"].transform(lambda x: random.choices([0, 1], weights=[x, 1 - x])[0])
        ...
        ].sample(n=10000, random_state=random_seed).reset_index(drop=True).copy()
        ...
        # get mapping from category to index
        imagenames_categories = dict(
            zip(df["GalaxyID"].astype(str), df["TARGET"])
        )
        ...
        image_paths = np.array(
            [
                "data/galaxyzoo/images_training_rev1/"
                + x
                + ".jpg"
                for x in image_names
            ]
        ...
        # get label for image names
        image_names_labels = np.array(
            [imagenames_categories[x] for x in image_names]
        )
        # get prediction for image names
        image_names_human_predictions = np.array(
            [
                image_name_to_single_participant_classification[x]
                for x in image_names
            ]
        )
