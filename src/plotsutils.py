...
markers_dict = {'CompareConfidence':"s",
...
palette = {q:sns.color_palette('colorblind')[i] for i,q in enumerate(sorted(markers_dict.keys(), reverse=True))}
...
def plot_single_classifier(df, classifier, figsize=(20, 12), title=""):
    ...
    for c in classifiers:
        ...
        (_, caps, _) = ax.errorbar(db_c["target_coverage"], db_c["coef_rob"], label=c, color=palette[c],yerr=(db_c["ci_rob_u"]-db_c["ci_rob_l"])/2, fmt='o', alpha=0.9, capsize=10,
    ...
    plt.savefig("{}/{}_{}_allcoeffs.png".format(img_fold, title, classifier), bbox_inches='tight', dpi=300)
    return ax
...
def plot_single_classifier_two_ys(df, classifier, figsize=(20, 12), title=""):
    ...
    for c in classifiers:
        db_c = db[db["method"] == c].copy()
        ax2.plot(db_c["target_coverage"], db_c["system_acc"], label=c, color="grey", alpha=0.4, marker=".", markersize=20, markeredgecolor = 'black',markeredgewidth = 0.5)
    ax2.set_ylabel("System accuracy", fontdict={"fontsize": 20}, rotation=-90, labelpad=30)
...
def plot_single_classifier_two_ys_V2(df, classifier, figsize=(20, 12), title=""):
    ...
    for c in classifiers:
        db_c = db[db["method"] == c].copy()
        ax2.plot(db_c["target_coverage"], db_c["system_acc"]/db_c["classifier_all_acc"]-1, label=c, color="grey", alpha=0.4, marker=".", markersize=20, markeredgecolor = 'black',markeredgewidth = 0.5)
    ax2.set_ylabel("Accuracy Increase", fontdict={"fontsize": 20}, rotation=-90, labelpad=30)
    # ax2.set_yticks(sorted([0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0, db["classifier_all_acc"].max()]))
    ax2.set_ylim(-(db_c["system_acc"]/db_c["classifier_all_acc"]).max()+.95, (db_c["system_acc"]/db_c["classifier_all_acc"]).max()-.95)
    ax2.set_yticks(np.linspace(-(db_c["system_acc"]/db_c["classifier_all_acc"]).max()+.95, (db_c["system_acc"]/db_c["classifier_all_acc"]).max()-.95, 5))
    ax2.set_yticklabels(["{:.1%}".format(x) if x!=0 else "{:.2f}".format(db_c["classifier_all_acc"].max()) for x in ax2.get_yticks()], fontsize=18)
